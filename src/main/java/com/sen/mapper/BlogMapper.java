...
import com.sen.entity.other.TimeLineBlog;
...
public interface BlogMapper extends BaseMapper<Blog> {
    ...
    @Results(id = "blogInfo", value = {
            ...
            @Result(property = "tags", column = "bl_id", many = @Many(select = "com.sen.mapper.BlogMapper.findTagsByBlog",
                    fetchType = FetchType.DEFAULT)),
            @Result(property = "type", column = "ty_id", one = @One(select = "com.sen.mapper.BlogMapper.findTypeByBlog",
                    fetchType = FetchType.DEFAULT)),
    })
    @Select("<script>" +
     ...
     *  根据type查找Blog(发布的)
     */
    @Select("select bl_id,title,outline,background_image,recommend,commentabled,published,views,ty_id,create_time,update_time " +
            "from t_blog " +
            "where ty_id = #{tyId} and published=true " +
            "order by create_time desc")
     ...
     *  根据tag查找Blog(发布的)
     */
    @Select("SELECT b.bl_id,b.title,b.outline,b.background_image,b.recommend,b.commentabled,b.published,b.views,b.ty_id,b.create_time,b.update_time " +
            "FROM t_blog AS b LEFT JOIN t_blog_tag AS bt ON b.bl_id=bt.bl_id " +
            "WHERE bt.ta_id=#{taId} and b.published=true " +
    ...
    @Results(value = {
            @Result(id = true, property = "blId", column = "bl_id"),
            @Result(property = "tagIds", column = "bl_id", one = @One(select = "com.sen.mapper.BlogMapper.findTagIdsById",
                    fetchType = FetchType.DEFAULT))
    })
    @Select("select * from t_blog where bl_id=#{blId} ")
